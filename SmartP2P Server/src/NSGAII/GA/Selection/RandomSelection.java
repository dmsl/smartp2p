...
public class RandomSelection{
    ...
    NSGAII.Solutions.Solution[] currentPopIPforGA;
    Random randomGen = null;
    int currentSubproblem = 0;
    ArrayList selectedParents = new ArrayList();
    
    /** Creates a new instance of RandomSelection */
    public RandomSelection(NSGAII.Solutions.Solution[] currentPopIPforGA, int currentSubproblem) {
    ...
    public ArrayList select(){
        ...
        int parent1Number = currentSubproblem;
        int parent2Number = currentSubproblem;
        
        while(parent1Number == currentSubproblem || parent2Number == currentSubproblem || parent1Number == parent2Number){
            
            randomGen = new Random(System.currentTimeMillis());            
            parent1Number = randomGen.nextInt(currentPopIPforGA.length);
            parent2Number = randomGen.nextInt(currentPopIPforGA.length);
            
        }
        
        parents.add((NSGAII.Solutions.Solution)currentPopIPforGA[parent1Number]);
        parents.add((NSGAII.Solutions.Solution)currentPopIPforGA[parent2Number]);        
        return parents;    
    }
